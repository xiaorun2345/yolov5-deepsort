...
LABEL_FILE_PATH = 'data/coco.names'
...
class PreprocessYOLO(object):
    ...
    def process(self, frame):
        ...
        image_raw, image_resized = self._load_and_resize(frame)
    ...
    def _load_and_resize(self, frame):
        ...
        image_resized = cv2.resize(frame, new_resolution)
...
class PostprocessYOLO(object):
    ...
    def process(self, outputs):
        ...
        for output, mask in zip(outputs, self.masks):
            anchors = [self.anchors[i] for i in mask]
            anchors = list(np.reshape(anchors,(6,-1)))
            ...
            t0 = time.time()
            all_boxes = []  #only output,not outputs
            output = output.view(batch*num_anchors, 5+num_classes, h*w).transpose(0,1).contiguous().view(5+num_classes, batch*num_anchors*h*w)

            #use CPU, so GPU more faster?
            ...
            if True:
                ...
                # print('      boxes filter : %f' % (t3-t0))
                print('---------------------------------')
        return out_boxes
        

    def process1(self, outputs, resolution_raw):
